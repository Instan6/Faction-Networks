...
-- Check if player is allowed to open a showroom (server-side check)
local function IsShowroomAccessAllowed(dealerName)
  local ok = lib.callback.await("jg-dealerships:server:check-showroom-access", false, dealerName)
  return ok == true
end
