...
function TriggerCallback(event, cb, ...)
    ...
    TriggerServerEvent("lb-radioapp:cb:" .. event, requestId, ...)
end
...
RegisterNetEvent("lb-radioapp:cb:response", function(requestId, ...)
...
function SendReactMessage(event, data)
    exports["lb-phone"]:SendCustomAppMessage("lb-radioapp", {
        action = event,
        data = data
    })
end
