...
function RegisterCallback(event, handler)
    RegisterNetEvent("lb-radioapp:cb:" .. event, function(requestId, ...)
        local src = source
        local params = { ... }
        local startTime = os.nanotime()

        local success, errorMessage = pcall(function()
            TriggerClientEvent("lb-radioapp:cb:response", src, requestId, handler(src, table.unpack(params)))
        ...
        if not success then
            ...
            TriggerClientEvent("lb-radioapp:cb:response", src, requestId, nil)
        end
    end)
end
